...
def show_result(DATASET, lable, Accuracy_List, Precision_List, Recall_List, AUC_List, AUPR_List):
    ...
    with open("{}/{}/results.txt".format(C_DIR,DATASET), 'w') as f:
...
def test_model(model, dataset_load, save_path, DATASET, LOSS, dataset="Train", lable="best", save=True):
    test_pbar = tqdm(
        enumerate(
...
def run():
    ...
    ar = torch.zeros([10, 20])
    dv = torch.device(params.DEVICE)
    # print(ar)
    ar = ar.to(dv)
    ...
    for i_fold in range(K_Fold):
        ...
        save_path = C_DIR + "/" + DATASET + "/{}".format(i_fold)
        note = ''
        ...
        trainset_test_stable_results, _, _, _, _, _ = test_model(model,train_dataset_load, save_path, DATASET, Loss,
                                                                 dataset="Train", lable="stable")
        validset_test_stable_results, _, _, _, _, _ = test_model(model, valid_dataset_load, save_path, DATASET, Loss,
                                                                 dataset="Valid", lable="stable")
        testset_test_stable_results, Accuracy_test, Precision_test, Recall_test, AUC_test, PRC_test = \
            test_model(model, test_dataset_load, save_path, DATASET, Loss, dataset="Test", lable="stable")
